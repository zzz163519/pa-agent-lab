...
- ADR-0020 and the Phase 5A contract accept the design for operator-only explicit corpus rollback and synthetic-only full-current-corpus Policy Assembly. The implementation plan is approved but implementation has not started and requires separate explicit Calvin start authorization. Phase 5A must stop before outbound payloads, ModelRuns, providers, Case-derived queries, real data, replay, or trading.
