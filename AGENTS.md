...
- ADR-0021 and `PHASE5B1_IMPLEMENTATION_AUTHORIZATION_V1.json` authorize only immutable Prompt Package governance, unperformed explicit activation/rollback capabilities, synthetic prepared payloads, and invented-response strict offline validation. The exact-content approval artifact remains immutable and separate. Performing package activation, provider/model calls, ModelRun/ProviderAttempt creation, production BrooksDecision creation, real data, replay, and trading remain unauthorized.
