...
- ADR-0023 preserves all accepted Phase 5 plumbing but separates engineering progress from Brooks semantic coverage and external-evaluation progress. It pauses V2 activation/preparation and provider calls until a separately accepted Phase 3C coverage baseline, Phase 4A2 expanded corpus activation, post-corpus assembly path, installed-CLI proof, and Phase 6 evaluation prerequisites exist. ADR-0023 grants no Source access, Doctrine mutation, implementation, provider, real-data, replay, or trading authority.
